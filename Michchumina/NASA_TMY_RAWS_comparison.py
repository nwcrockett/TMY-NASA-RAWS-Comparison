...
def graph_by_month_over_year(df_tmy, df_nasa, df_raws):
    ...
    for m, mn in month_dict.items():
        ...
        if mn == 'February' or mn == "March" or mn == 'April':
            df_nasa_month = df_nasa_month.loc[df_nasa_month["YEAR"] != 2008]
...
def graph_by_year(df_tmy, df_nasa, df_raws):
    ...
    for yr, yn in zip(unique_years_raws, unique_years_nasa):
        ...
        if yr == 2002 or yr == 2008 or yr == 2018:
            continue
...
def preprocess_raws_and_tmy_to_daily_sums(df_raws, df_tmy):
    ...
    tmy_solar_sums_by_day = df_tmy.groupby(["Year", "Month", "Day"])["GHI"].sum() / 1000
    date = []
...
if __name__ == "__main__":
    ...
    df_raws = read_csv("Lake Minchumina weather station.csv", header=6, skiprows=[7])
    df_tmy = read_csv("USA AK Minchumina (TMY3).csv", header=2)

    # possible unnecessary operation but avoids me having to check for negative values
